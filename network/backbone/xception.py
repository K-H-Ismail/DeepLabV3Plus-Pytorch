...
__all__ = ['xception_41', 'xception_65', 'xception_71']

model_ids = {
    'xception_41': '1CqaT2lhHzitxEOhWma1Cog-ssB8mWDWz',
    'xception_65': '1yuIpsZVLrpLxVN3h3wQK0MDaIRDI2gbw',
    'xception_71': '13FouJlBy6seRrw_uviuwqdoyE68ESJuU',
}
...
class StackBlocksDense(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        low_level_feat = self._blocks[0](x)
        x = self._blocks[1:](low_level_feat)
...
class Xception(torch.nn.Module):
    ...
    def __init__(self, blocks, num_classes=None, global_pool=True, 
        ...
        if global_pool:
            ...
            layers_output += [torch.nn.AdaptiveAvgPool2d(output_size=(1, 1))]
        if num_classes:
            layers_output += [torch.nn.Dropout2d(p=keep_prob, inplace=True),
                       torch.nn.Conv2d(blocks[-1].args[-1]['depth_list'][-1], 
                                       num_classes, 1)]
        self._layers = torch.nn.Sequential(*layers)
        self._layers_output = torch.nn.Sequential(*layers_output)
    ...
    def forward(self, x):
        ...
        Returns:
            Output tensor with stride equal to the specified output_stride.
        """
        low_level_feat, x = self._layers(x)
        x = self._layers_output(x)
        return {'out': x, 'low_level': low_level_feat}
...
def xception_41(num_classes=None,
    ...
    xception = Xception41(num_classes=num_classes, global_pool=global_pool, 
                          ...
                          scope=scope)
    if pretrained:
        dest = os.path.dirname(os.path.realpath(__file__)) + '/' + checkpoint_path
        if download and not os.path.exists(dest):
            download_file_from_google_drive(model_ids['xception_41'], dest)
...
def xception_65(num_classes=None,
    ...
    if pretrained:
        dest = os.path.dirname(os.path.realpath(__file__)) + '/' + checkpoint_path
        if download and not os.path.exists(dest):
            download_file_from_google_drive(model_ids['xception_65'], dest)
        _load_state_dict(xception, num_classes, checkpoint_path)
    return xception
...
def xception_71(num_classes=None,
    ...
    if pretrained:
        dest = os.path.dirname(os.path.realpath(__file__)) + '/' + checkpoint_path
        if download and not os.path.exists(dest):
            download_file_from_google_drive(model_ids['xception_71'], dest)
        _load_state_dict(xception, num_classes, checkpoint_path)
    return xception
...
def _load_state_dict(model, num_classes, checkpoint_path):
    ...
    checkpoint_path = os.path.dirname(os.path.realpath(__file__)) + '/' + checkpoint_path
